  ...
  try {
    ...
    if (reportType === "history" && dateFilter) {
      ...
      if (dateFilter.dateType === "specific") {
        dateText = `Specific Date: ${new Date(
    ...
    if (reportType === "history") {
      ...
      tableData = history.map((record) => [
        ...
        record.itemName.length > 30
          ? record.itemName.substring(0, 27) + "..."
          : record.itemName,
        record.category,
    ...
    if (finalY < 240) {
      ...
      doc.text(
        `Generated by: MICROTEK Inventory Management System v2.0`,
        20,
        finalY + 37
      );
      ...
      doc.text(
        `Report ID: MIK-${Date.now().toString().slice(-6)}`,
        110,
        finalY + 37
      );
    }
